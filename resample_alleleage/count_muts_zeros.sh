...
#!/bin/bash
#
## Count the number of mutations in each spectral class among 100 bins from a list of dated variants.
## Breaks out C-to-T triplets and calculates bin age as the mean among variants in that bin
## Differs from count_muts.sh by setting negative ages to zero in calculating average bin age
##
## Syntax:
...
##
## Output column order:
## A_C, ACC_ATC, A_G, A_T, C_A, CCC_CTC, C_G, CpG, C_T, G_A, G_C, G_T, T_A, T_C, TCC_TTC, TCT_TTT, T_G, bin_age
##
infile=$1
step=$2
upper=$(($step*100))
